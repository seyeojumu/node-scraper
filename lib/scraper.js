...
Scraper.prototype.parserForRequest = function(request, args) {
    var def = this._definition.parserDefinitionForRequest(request),
        parser = new Parser(def, this, args);
...
Scraper.prototype.setParentParser = function(parser) {
    this._parentParser = parser;
}

Scraper.prototype.getParentParser = function() {
...
Scraper.prototype.dump = function() {
    return require('sys').inspect({
        name: this.name,
        parsers: this.parsers.map(function(p) {
            return {
                id: p.id,
                args: p.args,
                status: p.response && p.response.statusCode,
                body: p.response && p.response.body && p.response.body.length,
                scrapers: p.scrapers.map(function(s) {
                    return s.dump();
                })
            }  
        })
